...
class Wav2NeRFNet(nn.Module):
    ...
    def calc_color_with_code(self, aud, bg_tensor, # modified
                             fg_vps, shape_code, appea_code, FGvp_embedder, 
        ...
        if fine_level:
            FGmlp_FGvp_rgb, FGmlp_FGvp_density = self.fine_fg_CD_predictor(ori_FGvp_embedder, ori_FGvd_embedder, bg=bg_tensor)
        else:
            FGmlp_FGvp_rgb, FGmlp_FGvp_density = self.fg_CD_predictor(ori_FGvp_embedder, ori_FGvd_embedder, bg=bg_tensor)
            
        fg_feat, _, _, ori_batch_weight = self.calc_color_func(fg_vps, FGmlp_FGvp_rgb,
                                                                                    FGmlp_FGvp_density,
                                                                                    FG_zdists,
                                                                                    FG_zvals)
        
        batch_size = fg_feat.size(0)
        merge_featmap = fg_feat.view(batch_size, self.featmap_nc, self.featmap_size, self.featmap_size) 
        merge_img, dwt, sync_mid = self.neural_render(merge_featmap, aud=aud)
    ...
    def _forward(
        ...
        if self.hier_sampling:
            ...
            f_ori_res, _= self.calc_color_with_code(
               aud, cur_shape_code, cur_appea_code, fine_FGvp_embedder, fine_FGvd_embedder, fine_FG_zdists, fine_FG_zvals, 
               fine_level=True
            )
