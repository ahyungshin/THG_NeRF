...
class MLPforNeRF(nn.Module):
    ...
    def _make_layers(self):
        ...
        # modified
        self.add_module("BG_layer_0", nn.Conv2d(3, self.res_nfeat, kernel_size=1, stride=1, padding=0))


    def forward(self, batch_embed_vps, batch_embed_vds, bg):
        '''
        ...
        bg = self._modules["BG_layer_0"](bg) # [5,3,32,32] --> [5,256,32,32]
        ...
        rgb = torch.cat([rgb[:,:,:,:-1], bg], dim=-1) # concat background
